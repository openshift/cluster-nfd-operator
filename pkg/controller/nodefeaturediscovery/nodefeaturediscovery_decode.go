...
func init() {
	...
	// manifests := GetAssetsFromPath("/opt/node-feature-discovery/assets/nfd-master")

	// r, _ := regexp.Compile(`\b(\w*kind:\w*)\B.*\b`)

	// for _, m := range manifests {
	// 	kind := r.FindString(string(m))
	// 	slce := strings.Split(kind, ":")
	// 	kind = strings.TrimSpace(slce[1])

	// 	switch kind {
	// 	case "ServiceAccount":
	// 		// res.ServiceAccount = res.ServiceAccount, corev1.ServiceAccount{})
	// 		// currentResource = len(res.ServiceAccount) - 1
	// 		// _, _, err := s.Decode(m, nil, &res.ServiceAccount[currentResource])
	// 		// panicIfError(err)
	// 	}
